...
} from "../../utils/utils-datetime";
import {
  calcChargeFinishTime,
  predict1HourVisitNum,
} from "../../utils/utils-charger";
import VisitNumGraph from "./VisitNumGraph";

const DetailedInfoView = memo(
  ({
    station,
    toggleView,
  }: {
    ...
    const getInfo = async <T = StationDetailed,>(): Promise<T> => {
      const { data } = await axios.get<T>(
        `/api/stations/${station.statId}?brief=no`
      );
      return data;
    };
    ...
    const closeBtnHandler = useCallback(async () => {
      ...
      try {
        const response = await axios.put(
          `/api/stations/${station.statId}/view-num/down`
        );
      } catch (error) {
      ...
      createPortal(
        ...
        >
          ...
          <div className="w-full h-auto flex flex-col gap-4 p-4 mb-[100px]">
            ...
            <div className="detail-info-section ">
              <div>
                ...
                <h2 className="text-[#A8A8A8] font-semibold text-sm">
                  {data.info.addr}
                </h2>
              </div>
              <div className="flex flex-col gap-2">
                <div className="info-with-icon">
                  <IconEye />
                  <span>
                    현재 <strong>{data.demandInfo?.viewNum || 0}명</strong>이
                    조회중이에요
                  </span>
                </div>
                <div className="info-with-icon">
                  ...
                  <span>
                    지난 30분 동안{" "}
                    <strong>
                      {data.demandInfo?.departsIn30m.length || 0}명
                    </strong>
                    이 출발했어요
                  </span>
                </div>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  {predictResult !== undefined && (
                    <div className="info-with-icon">
                      ...
                      <span>
                        1시간 내로{" "}
                        <strong>{predict1HourVisitNum(station)}명</strong>이
                        사용할 것으로 예상돼요
                      </span>
                    </div>
                  )}
                  {/* 설명 */}
                  {altText && (
                    <span className="text-[#A8A8A8] font-semibold text-sm">
                      {altText}
                    </span>
                  )}
                </div>
            ...
            <div className="detail-info-section">
              ...
              <div className="flex flex-col gap-3">
                {Array.from(
                  new Set(data.chargers.map((chgr) => chgr.output))
                ).map((output) => {
                  if (output.length === 0)
                    return (
                  ...
                  return (
                    <div key={output}>
                      ...
                      <div className="text-xs text-[#A1A1A1] font-light">
                        {
                          ChargerTypes[
                            data.chargers.filter(
                              (chgr) => chgr.output === output
                            )[0].chgerType as keyof typeof ChargerTypes
                          ]
                        }
                      </div>
                      {/* 충전기 상태*/}
                        ...
                        {data.chargers
                          ...
                          .map((chgr) => {
                            ...
                            const isLastTsdtFresh =
                              ...
                              chgr.lastTsdt !== "" &&
                              Date.now() -
                                yyyyMMddHHmmssToDateTime(chgr.lastTsdt) <
                                16 * 60 * 60 * 1000;

                            return (
                              <div
                                className="flex flex-col bg-[#F4F4F4] rounded-md p-2"
                                key={chgr.chgerId}
                              >
                                {/* 상태 */}
                                <span
                                ...
                                >
                                  {
                                    ChargerStatus[
                                      ("0" +
                                        chgr.stat) as keyof typeof ChargerStatus
                                    ]
                                  }
                                </span>
                                {/* 마지막 충전종료 시각 */}
            ...
            <div className="detail-info-section ">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="info-with-icon">
                  <IconParking />
                  <span>
                    {(data.info.parkingFree && data.info.parkingFree === "Y"
                      ? "무료주차"
                      : "유료주차") || "무료주차여부 미제공"}
                  </span>
                </div>
            ...
            {data.demandInfo && data.demandInfo.hourlyVisitNum.length > 0 && (
              <div className="detail-info-section">
                <div>
                  <h1>이용객수 예상 그래프</h1>
                  <h2 className="text-[#A8A8A8] font-semibold text-xs">
                    지난 30일동안의 충전이력을 기반으로 예측한 금일 이용객수
                    그래프예요
                  </h2>
          ...
          <div className="fixed bottom-0 w-full h-auto p-4 bg-slate-100 md:max-w-[375px]">
            <button
              className="w-full h-auto info-brief-btn bg-black text-white p-4 cursor-pointer"
              onClick={showModal}
            >
              경로안내
            </button>
