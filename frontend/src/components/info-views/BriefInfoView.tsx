...
const BriefInfoView = memo(function ({ station }: { station: StationSummarized }) {
  ...
  const detailInfoBtnHandler = useCallback(async () => {
    try {
      const response = await apiClient.put(`/api/stations/${station.statId}/view-num/up`);
    } catch (error) {
  ...
  /** 상세정보 창 open 상태 */
  const [isDetailedInfoViewOpen, setIsDetailedInfoViewOpen] = useState<boolean>(false);

  const toggleIsDetailedInfoView = useCallback(() => {
    setIsDetailedInfoViewOpen(!isDetailedInfoViewOpen);
  }, [isDetailedInfoViewOpen]);

  return data ? (
    <>
      <article id="brief-info" className="absolute bottom-0 left-0 flex w-full bg-white z-[2] p-4 gap-3" ref={infoArea}>
        {/* 충전소 정보 wrapper*/}
        <div className="flex-1 flex flex-col justify-between gap-2">
          {/* 충전소명 */}
          <h1 className="font-extrabold text-lg leading-6 mb-1">{data.info.statNm}</h1>
          {/* 충전소 상태 (이용가능/전체대수) */}
        ...
        <div className="w-1/3 flex flex-col justify-center gap-2">
          {/* 경로안내 버튼 */}
          <button className="info-brief-btn bg-black text-white" onClick={naviBtnHandler}>
            경로안내
          </button>
          {/* 상세정보 버튼 */}
          <button className="info-brief-btn bg-[#ECECEC] text-[#535353]" onClick={detailInfoBtnHandler}>
            상세정보
          </button>
        </div>
      </article>
      {isDetailedInfoViewOpen && <DetailedInfoView station={station} toggleView={toggleIsDetailedInfoView} />}
    </>
  ) : (
    <></>
  );
